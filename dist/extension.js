...
// src/plugins/github-alert/parser.mjs
function parseTokenForGitHubAlertName(token) {
  ...
  if (!firstLine.startsWith("[!"))
    return;
  const regexp = /^\[[!](note|warning|important|caution|tip)\]$/;
...
function gitHubAlert(md) {
  ...
  md.block.ruler.at(
    ...
    (state, startLine, endLine, silent) => {
      ...
      const gitHubAlertName = parseTokenForGitHubAlertName(inlineToken);
      if (!gitHubAlertName)
        return result;
      const gitHubAlert2 = GitHubAlerts.find((alert) => alert.name === gitHubAlertName);
      inlineToken.content = inlineToken.content.split("\n").slice(1).join("\n");
      tokens[i].meta = { isGitHubAlert: true, gitHubAlert: gitHubAlert2 };
      lastToken.meta = { isGitHubAlert: true };
      return result;
    },
    {
      alt: [
...
// src/plugins/obsidian-callout/parser.mjs
function parseTokenForObsidianCalloutAlias(token) {
  if (token === void 0 || token.type !== "inline")
    return;
  const lines = token.content.split("\n");
  if (lines.length <= 1)
...
function renderBlockquoteOpen2(tokens, idx, options, env, self) {
  const token = tokens[idx];
  if (token.meta && token.meta.obsidianCallout)
    return renderOpening2(token.meta.obsidianCallout);
  return self.renderToken(tokens, idx, options);
}
function renderBlockquoteClose2(tokens, idx, options, env, self) {
  const token = tokens[idx];
  if (token.meta && token.meta.isObsidianCallout)
    return renderClosing2();
  return self.renderToken(tokens, idx, options);
}
function obsidianCallout(md) {
  md.renderer.rules.blockquote_open = renderBlockquoteOpen2;
  md.renderer.rules.blockquote_close = renderBlockquoteClose2;
  md.block.ruler.at(
    "blockquote",
    (state, startLine, endLine, silent) => {
      const result = blockquote(state, startLine, endLine, silent);
...
function activate(context) {
  return {
     ...
     */
    extendMarkdownIt(md) {
      return md.use(gitHubAlert).use(obsidianCallout);
    }
  };
}
function deactivate() {
